...
CairnDB is a serverless database engine built over blob storage.
There are no long-running servers: the bucket is the arbiter.
It is designed for low/medium workloads where higher latency is not a problem.
In return, operating CairnDB is extremely cheap.
...
<!-- TODO -->
Cooperative concurrent programming introductory paragraph.
WAL, atomic writes, transactions, uniqueness constraints, locks, indexes, materialized views.

### Layer 0 — Objects

Conditional key-value store.
Implements a few write semantics:
  - Put: write always.
  - Put-If-Match: write only if the ETag matches, receiving new ETag or None if race lost.
  - Put-If-Absent: write only if absent, receiving ETag or None if race lost.

### Layer 1 — Coordination

#### claim — unique constraint

Put-if-absent with read-back of the winner.
Duplicate callers **converge**: losers get winner's value.
Claims are immutable.

#### lease — expiring ownership with fencing
Epoch-fenced ownership document: `{epoch, holder, deadline_at, state}`.
As long as the arbiter's lease object's epoch matches, the acquired lease is evidence of ownership.
Writes through the lease are CAS enforced by ETag guards.
If ETag does not match but epoch and owner still match, this is a cooperative write.
If epoch advanced, the lease was lost.
So the epoch helps classify the fencing over protected content.
Note that lease expiry is not enforced: the owner can optimistically continue, and if the lease was not acquired by someone else, the owner effectively is still the owner.
...
#### (Named) Logs
<!-- TODO: What are logs -->
Logs are globally densely ordered ledgers.
Any writer appends to a log through the commit protocol; there is no
sequencer process — the bucket's put-if-absent arbitration sequences each log.
They use generic conditional-object for fencing.

#### Transactions — multi-key atomicity via the log

The one thing raw blob CAS cannot do: change several keyed objects atomically.
The engine uses a dedicated system log as the transaction coordinator:
...
Projections overs logs are like materialised views.
Snapshots provide efficient memoisation for projections and are logged through the replay/updater machinery.
Point-in-time views bootstrap from a snapshot and replay logs up to recovery point.
...
- Leases: mutual exclusion within clock skew ≪ ttl; fencing by epoch + CAS makes even a paused holder safe (its writes fail).
- Logs: linearizable appends, total order per log, durable ack.
- Transactions: optimistic serializable w.r.t. other transactions in the same engine.
- Projections: eventually consistent, monotonic per client, opt-in read-your-writes via `wait_for`.
...
- **Named logs** reroute the commit protocol under `logs/{name}/` via the
  generic object API — same Committer, one dense sequence per log.
...
<!-- TODO: delete ? -->
A database engine decomposes into a WAL, a concurrency-control layer, derived state, and vacuum — CairnDB exposes each
